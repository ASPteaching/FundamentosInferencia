...
- [Berrendero, José. Fundamentos de Estadística](https://verso.mat.uam.es/~joser.berrendero/libro-est/)
...
<!--chapter:end:01-probabilidad.Rmd-->
...
<!--chapter:end:02-variablesAleatorias.Rmd-->
...
<!--chapter:end:03-distribucionesNotables.Rmd-->
...
<!--chapter:end:04-vectoresAleatorios.Rmd-->
...
<!--chapter:end:05-grandesMuestras.Rmd-->
...
<!--chapter:end:06-introInferenciaExsR.Rmd-->
...
<!--chapter:end:07-estimacionPuntual.Rmd-->
...
<!--chapter:end:08-estimacionIntervalos.Rmd-->
...
kable(res, digits = c(0, 4))
# kableExtra::kable(res, digits = c(0, 4), format = "html") %>%
...
```{r, echo=FALSE}
m <- 7
meansTab <- data.frame(Media =numeric(11), 
                       ProbsX =numeric(11))
meanX<- meansTab[, 1] <- seq(6.7, 9.7, by=0.3)
meansTab[, 2] <- round(pnorm(meanX, mean=m, sd=0.6),4)

meanInt <- data.frame(intMedLow =numeric(11),
                       intMedSup =numeric(11),
                       ProbsInt =numeric(11))

calcProbInt <- function(linf, lsup, m){
  pInt <- pnorm(lsup, mean=m, sd=0.6) - pnorm(linf, mean=m, sd=0.6)
}
                                             
meanInt[1,1] <- -Inf
meanInt[1,2] <- meansTab[1]
meanInt[1,3] <- pnorm(meanInt[1,2], mean=m, sd=0.6)
for (i in 1:(nrow(meansTab)-1)){
  linf = meanInt[i+1,1] <- meanX[i]
  lsup = meanInt[i+1,2] <- meanX[i+1]
  meanInt[i+1,3] <-calcProbInt (linf, lsup, m)
}
twoTables <- cbind(meansTab, meanInt)
```
...
pretty_table(
  twoTables,
  digits = c(1, 4, 1,1, 4),
  col.names = c("$X$", "$P(X\\leq x)$",  "$X-\\epsilon$", "$X+\\epsilon$", "$P(X)$")
)
```
...
calcProbInt <- function(linf, lsup){
  pInt <- pnorm(lsup, mean=m, sd=0.6) - pnorm(linf, mean=8.5, sd=0.6)
}
...
for (i in 1:(nrow(meansTab)-1)){
  linf = meanInt[i+1,1] <- meanX[i]
  lsup = meanInt[i+1,2] <- meanX[i+1]
  meanInt[i+1,3] <-calcProbInt (linf, lsup)
}
twoTables <- cbind(meansTab, meanInt)
```
...
### Caso 1: Una región con número de hembras con baja probabilidad bajo $\mathrm{H}_{0}$
...
### Región crítica frente a Estadístico de Test
...
- [Error de tipo I, de tipo II y potencia en un Z-test (1 muestra normal, varianza conocida)](https://www.grbio.eu/statmedia/Statmedia_3/)

- [Errores de tipo I, II, potencia y p-valor en el test normal de una muestra, varianza conocida](https://www.grbio.eu/statmedia/Statmedia_4/)
...
<!--chapter:end:09-pruebasHipotesis.Rmd-->
...
<!--chapter:end:10-Construccion_de_Tests.Rmd-->
...
<!--chapter:end:11-inferenciaAplicada.Rmd-->
