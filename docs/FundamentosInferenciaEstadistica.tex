...
\subsubsection{Ejemplo 1: Bondad de ajuste}\label{ejemplo-1-bondad-de-ajuste}
...
\subsubsection{Ejemplo 2: Ajuste de modelo genético}\label{ejemplo-2-ajuste-de-modelo-genuxe9tico}
...
\subsubsection{Ejemplo 3: Ajuste de modelo genético con probabilidades desconocidas}\label{ejemplo-3-ajuste-de-modelo-genuxe9tico-con-probabilidades-desconocidas}
...
\subsubsection{Ejemplo 4: Ajuste a una distribución normal}\label{ejemplo-4-ajuste-a-una-distribuciuxf3n-normal}
...
\subsubsection{Ejemplo 5: Relación entre nivel de estudios y preferencias}\label{ejemplo-5-relaciuxf3n-entre-nivel-de-estudios-y-preferencias}
...
\subsubsection{Ejemplo 6: Homogeneidad en los grupos sanguíneos}\label{ejemplo-6-homogeneidad-en-los-grupos-sanguuxedneos}
...
Por tanto, \textbf{no se rechaza la hipótesis nula de homogeneidad}, y los datos son compatibles con la hipótesis de que la probabilidad de pertenecer a cada grupo sanguíneo es la misma en las tres razas consideradas.

\subsection{Ejemplos resueltos en R}\label{ejemplos-resueltos-en-r}
...
\subsubsection{Ejemplo 1: Bondad de ajuste}\label{ejemplo-1-bondad-de-ajuste-1}

\textbf{Datos (200 días).} Se contrasta \(H_0: X \sim \text{Poisson}(\lambda=2)\).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n }\OtherTok{\textless{}{-}} \DecValTok{200}
\NormalTok{obs\_raw }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{0}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{22}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{1}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{53}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{2}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{58}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{3}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{39}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{4}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{20}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{5}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{5}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{6}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{2}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{7}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{)}

\CommentTok{\# Agrupamos en categoría "\textgreater{}=5" para evitar esperadas pequeñas}
\NormalTok{obs }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{0}\StringTok{\textasciigrave{}}\OtherTok{=}\NormalTok{obs\_raw[}\StringTok{"0"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{1}\StringTok{\textasciigrave{}}\OtherTok{=}\NormalTok{obs\_raw[}\StringTok{"1"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{2}\StringTok{\textasciigrave{}}\OtherTok{=}\NormalTok{obs\_raw[}\StringTok{"2"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{3}\StringTok{\textasciigrave{}}\OtherTok{=}\NormalTok{obs\_raw[}\StringTok{"3"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{4}\StringTok{\textasciigrave{}}\OtherTok{=}\NormalTok{obs\_raw[}\StringTok{"4"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{\textgreater{}=5}\StringTok{\textasciigrave{}}\OtherTok{=}\FunctionTok{sum}\NormalTok{(obs\_raw[}\FunctionTok{c}\NormalTok{(}\StringTok{"5"}\NormalTok{,}\StringTok{"6"}\NormalTok{,}\StringTok{"7"}\NormalTok{)]))}
...
\CommentTok{\# Probabilidades bajo Poisson(2): 0..4 y \textgreater{}=5}
\NormalTok{p0\_4 }\OtherTok{\textless{}{-}} \FunctionTok{dpois}\NormalTok{(}\DecValTok{0}\SpecialCharTok{:}\DecValTok{4}\NormalTok{, }\AttributeTok{lambda =}\NormalTok{ lambda)}
\NormalTok{p\_ge5 }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{ppois}\NormalTok{(}\DecValTok{4}\NormalTok{, }\AttributeTok{lambda =}\NormalTok{ lambda)}
\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(p0\_4, p\_ge5)}
\FunctionTok{names}\NormalTok{(p) }\OtherTok{\textless{}{-}} \FunctionTok{names}\NormalTok{(obs)}

\CommentTok{\# Comprobación: suman 1}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Test chi{-}cuadrado (probabilidades completamente especificadas)}
\NormalTok{test1 }\OtherTok{\textless{}{-}} \FunctionTok{chisq.test}\NormalTok{(}\AttributeTok{x =}\NormalTok{ obs, }\AttributeTok{p =}\NormalTok{ p, }\AttributeTok{rescale.p =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{test1}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  obs
## X-squared = 2.3031, df = 5, p-value = 0.8058
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{expected }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(test1}\SpecialCharTok{$}\NormalTok{expected)}
\NormalTok{stat\_contrib }\OtherTok{\textless{}{-}}\NormalTok{ (obs }\SpecialCharTok{{-}}\NormalTok{ expected)}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{/}\NormalTok{ expected}

\NormalTok{res1 }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{Categoria =} \FunctionTok{names}\NormalTok{(obs),}
  \AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs),}
  \AttributeTok{p0 =} \FunctionTok{as.numeric}\NormalTok{(p),}
  \AttributeTok{Esperado =}\NormalTok{ expected,}
  \AttributeTok{Contribucion =}\NormalTok{ stat\_contrib}
\NormalTok{)}
\NormalTok{res1}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
##     Categoria Observado         p0 Esperado Contribucion
...
## >=5       >=5         8 0.05265302 10.53060   0.60812791
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sum}\NormalTok{(res1}\SpecialCharTok{$}\NormalTok{Contribucion)  }\CommentTok{\# Debe coincidir con el estadístico}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{op }\OtherTok{\textless{}{-}} \FunctionTok{par}\NormalTok{(}\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{))}
\NormalTok{bp }\OtherTok{\textless{}{-}} \FunctionTok{barplot}\NormalTok{(}\FunctionTok{rbind}\NormalTok{(}\AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs), }\AttributeTok{Esperado =}\NormalTok{ expected),}
              \AttributeTok{beside =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{las =} \DecValTok{2}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Frecuencia"}\NormalTok{,}
              \AttributeTok{main =} \StringTok{"Ejemplo 1: Observado vs Esperado (Poisson(2))"}\NormalTok{)}
\FunctionTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Observado"}\NormalTok{,}\StringTok{"Esperado"}\NormalTok{), }\AttributeTok{bty =} \StringTok{"n"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej1-plot-1.pdf}}
...
\subsubsection{Ejemplo 2: Ajuste de modelo genético}\label{ejemplo-2-ajuste-de-modelo-genuxe9tico-1}

Modelo: proporciones \(1:2:1\) (multinomial con \(p=(0.25,0.5,0.25)\)).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{obs }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\AttributeTok{Blanca =} \DecValTok{112}\NormalTok{, }\AttributeTok{Rosa =} \DecValTok{242}\NormalTok{, }\AttributeTok{Roja =} \DecValTok{112}\NormalTok{)}
\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FloatTok{0.25}\NormalTok{, }\FloatTok{0.50}\NormalTok{, }\FloatTok{0.25}\NormalTok{)}

\NormalTok{test2 }\OtherTok{\textless{}{-}} \FunctionTok{chisq.test}\NormalTok{(}\AttributeTok{x =}\NormalTok{ obs, }\AttributeTok{p =}\NormalTok{ p, }\AttributeTok{rescale.p =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{test2}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  obs
## X-squared = 0.69528, df = 2, p-value = 0.7064
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{expected }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(test2}\SpecialCharTok{$}\NormalTok{expected)}

\FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{Categoria =} \FunctionTok{names}\NormalTok{(obs),}
  \AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs),}
  \AttributeTok{Esperado  =}\NormalTok{ expected,}
  \AttributeTok{Contribucion =}\NormalTok{ (obs}\SpecialCharTok{{-}}\NormalTok{expected)}\SpecialCharTok{\^{}}\DecValTok{2}\SpecialCharTok{/}\NormalTok{expected}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## Roja        Roja       112    116.5    0.1738197
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{barplot}\NormalTok{(}\FunctionTok{rbind}\NormalTok{(}\AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs), }\AttributeTok{Esperado =}\NormalTok{ expected),}
        \AttributeTok{beside =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{las =} \DecValTok{2}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Frecuencia"}\NormalTok{,}
        \AttributeTok{main =} \StringTok{"Ejemplo 2: Observado vs Esperado (1:2:1)"}\NormalTok{)}
\FunctionTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Observado"}\NormalTok{,}\StringTok{"Esperado"}\NormalTok{), }\AttributeTok{bty =} \StringTok{"n"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej2-tabla-plot-1.pdf}}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Ejemplo 3: Ajuste de modelo genético con probabilidades desconocidas}\label{ejemplo-3-ajuste-de-modelo-genuxe9tico-con-probabilidades-desconocidas-1}

Se contrasta el ajuste a un \textbf{modelo Hardy--Weinberg} para alelos \(M/N\):
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n1 }\OtherTok{\textless{}{-}} \DecValTok{1787}
\NormalTok{n2 }\OtherTok{\textless{}{-}} \DecValTok{3039}
\NormalTok{n3 }\OtherTok{\textless{}{-}} \DecValTok{1303}
\NormalTok{n  }\OtherTok{\textless{}{-}}\NormalTok{ n1 }\SpecialCharTok{+}\NormalTok{ n2 }\SpecialCharTok{+}\NormalTok{ n3}

\NormalTok{p\_hat }\OtherTok{\textless{}{-}}\NormalTok{ (}\DecValTok{2}\SpecialCharTok{*}\NormalTok{n1 }\SpecialCharTok{+}\NormalTok{ n2) }\SpecialCharTok{/}\NormalTok{ (}\DecValTok{2}\SpecialCharTok{*}\NormalTok{n)}
\NormalTok{p\_hat}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p\_model }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\AttributeTok{M =}\NormalTok{ p\_hat}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{,}
             \StringTok{\textasciigrave{}}\AttributeTok{M{-}N}\StringTok{\textasciigrave{}} \OtherTok{=} \DecValTok{2}\SpecialCharTok{*}\NormalTok{p\_hat}\SpecialCharTok{*}\NormalTok{(}\DecValTok{1}\SpecialCharTok{{-}}\NormalTok{p\_hat),}
             \AttributeTok{N =}\NormalTok{ (}\DecValTok{1}\SpecialCharTok{{-}}\NormalTok{p\_hat)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}

\NormalTok{expected }\OtherTok{\textless{}{-}}\NormalTok{ n }\SpecialCharTok{*}\NormalTok{ p\_model}
\NormalTok{obs }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\AttributeTok{M =}\NormalTok{ n1, }\StringTok{\textasciigrave{}}\AttributeTok{M{-}N}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ n2, }\AttributeTok{N =}\NormalTok{ n3)}

\CommentTok{\# Estadístico de Pearson}
\NormalTok{D2 }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{((obs }\SpecialCharTok{{-}}\NormalTok{ expected)}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{/}\NormalTok{ expected)}

\CommentTok{\# Grados de libertad ajustados: k {-} 1 {-} m = 3 {-} 1 {-} 1 = 1}
\NormalTok{df }\OtherTok{\textless{}{-}} \DecValTok{1}
\NormalTok{p\_value }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{pchisq}\NormalTok{(D2, }\AttributeTok{df =}\NormalTok{ df)}

\FunctionTok{list}\NormalTok{(}
  \AttributeTok{p\_hat =}\NormalTok{ p\_hat,}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{barplot}\NormalTok{(}\FunctionTok{rbind}\NormalTok{(}\AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs), }\AttributeTok{Esperado =} \FunctionTok{as.numeric}\NormalTok{(expected)),}
        \AttributeTok{beside =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{las =} \DecValTok{2}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Frecuencia"}\NormalTok{,}
        \AttributeTok{main =} \StringTok{"Ejemplo 3: Observado vs Esperado (modelo con p estimado)"}\NormalTok{)}
\FunctionTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Observado"}\NormalTok{,}\StringTok{"Esperado"}\NormalTok{), }\AttributeTok{bty =} \StringTok{"n"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej3-plot-1.pdf}}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Ejemplo 4: Ajuste a una distribución normal}\label{ejemplo-4-ajuste-a-una-distribuciuxf3n-normal-1}
...
Queremos contrastar \(H_0: X \sim N(\mu,\sigma)\), con \(\mu\) y \(\sigma\) \textbf{desconocidos} (se estiman).

\begin{quote}
Importante: como se estiman 2 parámetros, el contraste usa \textbf{\(k-1-m\)} grados de libertad tras posibles fusiones de clases.
\end{quote}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{obs\_raw }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{\textless{}=55}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{4}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{55{-}60}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{17}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{60{-}65}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{45}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{65{-}70}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{67}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{70{-}75}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{53}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{75{-}80}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{15}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{80{-}85}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{10}\NormalTok{)}

\NormalTok{n }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(obs\_raw)}


\NormalTok{mu\_hat }\OtherTok{\textless{}{-}} \FloatTok{68.52}
\NormalTok{sigma\_hat }\OtherTok{\textless{}{-}} \FloatTok{6.44}
\NormalTok{mu\_hat; sigma\_hat}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Intervalos: ({-}Inf,55], (55,60], ..., (80,Inf)}
\NormalTok{breaks }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\ConstantTok{Inf}\NormalTok{, }\DecValTok{55}\NormalTok{, }\DecValTok{60}\NormalTok{, }\DecValTok{65}\NormalTok{, }\DecValTok{70}\NormalTok{, }\DecValTok{75}\NormalTok{, }\DecValTok{80}\NormalTok{, }\ConstantTok{Inf}\NormalTok{)}

\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{numeric}\NormalTok{(}\FunctionTok{length}\NormalTok{(breaks)}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(p))\{}
\NormalTok{  a }\OtherTok{\textless{}{-}}\NormalTok{ breaks[i]; b }\OtherTok{\textless{}{-}}\NormalTok{ breaks[i}\SpecialCharTok{+}\DecValTok{1}\NormalTok{]}
\NormalTok{  p[i] }\OtherTok{\textless{}{-}} \FunctionTok{pnorm}\NormalTok{(b, }\AttributeTok{mean =}\NormalTok{ mu\_hat, }\AttributeTok{sd =}\NormalTok{ sigma\_hat) }\SpecialCharTok{{-}} \FunctionTok{pnorm}\NormalTok{(a, }\AttributeTok{mean =}\NormalTok{ mu\_hat, }\AttributeTok{sd =}\NormalTok{ sigma\_hat)}
\NormalTok{\}}
\FunctionTok{names}\NormalTok{(p) }\OtherTok{\textless{}{-}} \FunctionTok{names}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{\textless{}=55}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{55{-}60}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{60{-}65}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{65{-}70}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{70{-}75}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{75{-}80}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{, }\StringTok{\textasciigrave{}}\AttributeTok{\textgreater{}=80}\StringTok{\textasciigrave{}}\OtherTok{=}\DecValTok{1}\NormalTok{))}
\NormalTok{p}
\end{Highlighting}
\end{Shaded}
...
## 0.01789176 0.07502903 0.19941155 0.29854938 0.25196129 0.11983199 0.03732501
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{expected\_raw }\OtherTok{\textless{}{-}}\NormalTok{ n }\SpecialCharTok{*}\NormalTok{ p}

\CommentTok{\# Fusionamos las dos primeras clases si alguna esperada \textless{} 5 (en el texto se fusionan)}
\NormalTok{obs }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{\textless{}=60}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ obs\_raw[}\StringTok{"\textless{}=55"}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ obs\_raw[}\StringTok{"55{-}60"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{60{-}65}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ obs\_raw[}\StringTok{"60{-}65"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{65{-}70}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ obs\_raw[}\StringTok{"65{-}70"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{70{-}75}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ obs\_raw[}\StringTok{"70{-}75"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{75{-}80}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ obs\_raw[}\StringTok{"75{-}80"}\NormalTok{],}
         \StringTok{\textasciigrave{}}\AttributeTok{\textgreater{}=80}\StringTok{\textasciigrave{}}  \OtherTok{=}\NormalTok{ obs\_raw[}\StringTok{"80{-}85"}\NormalTok{])}

\NormalTok{expected }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{\textless{}=60}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ expected\_raw[}\DecValTok{1}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ expected\_raw[}\DecValTok{2}\NormalTok{],}
              \StringTok{\textasciigrave{}}\AttributeTok{60{-}65}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ expected\_raw[}\DecValTok{3}\NormalTok{],}
              \StringTok{\textasciigrave{}}\AttributeTok{65{-}70}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ expected\_raw[}\DecValTok{4}\NormalTok{],}
              \StringTok{\textasciigrave{}}\AttributeTok{70{-}75}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ expected\_raw[}\DecValTok{5}\NormalTok{],}
              \StringTok{\textasciigrave{}}\AttributeTok{75{-}80}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ expected\_raw[}\DecValTok{6}\NormalTok{],}
              \StringTok{\textasciigrave{}}\AttributeTok{\textgreater{}=80}\StringTok{\textasciigrave{}}  \OtherTok{=}\NormalTok{ expected\_raw[}\DecValTok{7}\NormalTok{])}

\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{Categoria =} \FunctionTok{names}\NormalTok{(obs),}
           \AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs),}
           \AttributeTok{Esperado  =} \FunctionTok{as.numeric}\NormalTok{(expected))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{k }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(obs)        }\CommentTok{\# número final de clases}
\NormalTok{m }\OtherTok{\textless{}{-}} \DecValTok{2}                  \CommentTok{\# mu y sigma estimados}
\NormalTok{df }\OtherTok{\textless{}{-}}\NormalTok{ k }\SpecialCharTok{{-}} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ m}

\NormalTok{p\_value }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{pchisq}\NormalTok{(D2, }\AttributeTok{df =}\NormalTok{ df)}

\FunctionTok{list}\NormalTok{(}\AttributeTok{D2 =}\NormalTok{ D2, }\AttributeTok{df =}\NormalTok{ df, }\AttributeTok{p\_value =}\NormalTok{ p\_value)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{barplot}\NormalTok{(}\FunctionTok{rbind}\NormalTok{(}\AttributeTok{Observado =} \FunctionTok{as.numeric}\NormalTok{(obs), }\AttributeTok{Esperado =} \FunctionTok{as.numeric}\NormalTok{(expected)),}
        \AttributeTok{beside =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{las =} \DecValTok{2}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Frecuencia"}\NormalTok{,}
        \AttributeTok{main =} \StringTok{"Ejemplo 4: Observado vs Esperado (Normal con parámetros estimados)"}\NormalTok{)}
\FunctionTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Observado"}\NormalTok{,}\StringTok{"Esperado"}\NormalTok{), }\AttributeTok{bty =} \StringTok{"n"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej4-plot-1.pdf}}

\subsubsection{Ejemplo 5: Relación entre nivel de estudios y preferencias}\label{ejemplo-5-relaciuxf3n-entre-nivel-de-estudios-y-preferencias-1}
...
\NormalTok{tab }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}
  \FunctionTok{c}\NormalTok{(}\DecValTok{31}\NormalTok{,}\DecValTok{49}\NormalTok{,}\DecValTok{18}\NormalTok{,}\DecValTok{12}\NormalTok{,}
    \DecValTok{11}\NormalTok{,}\DecValTok{59}\NormalTok{,}\DecValTok{26}\NormalTok{,}\DecValTok{25}\NormalTok{,}
    \DecValTok{12}\NormalTok{,}\DecValTok{51}\NormalTok{,}\DecValTok{31}\NormalTok{,}\DecValTok{36}\NormalTok{),}
  \AttributeTok{nrow =} \DecValTok{3}\NormalTok{, }\AttributeTok{byrow =} \ConstantTok{TRUE}
\NormalTok{)}
\FunctionTok{rownames}\NormalTok{(tab) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Elementales"}\NormalTok{,}\StringTok{"Nivel medio"}\NormalTok{,}\StringTok{"Superiores"}\NormalTok{)}
\FunctionTok{colnames}\NormalTok{(tab) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Ninguno"}\NormalTok{,}\StringTok{"Motos"}\NormalTok{,}\StringTok{"C. normal"}\NormalTok{,}\StringTok{"C. potente"}\NormalTok{)}
\NormalTok{tab}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##             Ninguno Motos C. normal C. potente
## Elementales      31    49        18         12
## Nivel medio      11    59        26         25
## Superiores       12    51        31         36
\end{verbatim}
...
\begin{verbatim}
##             Ninguno Motos C. normal C. potente Sum
## Elementales      31    49        18         12 110
## Nivel medio      11    59        26         25 121
## Superiores       12    51        31         36 130
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test5 }\OtherTok{\textless{}{-}} \FunctionTok{chisq.test}\NormalTok{(tab, }\AttributeTok{correct =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{test5}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  tab
## X-squared = 29.763, df = 6, p-value = 4.361e-05
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test5}\SpecialCharTok{$}\NormalTok{expected}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mosaicplot}\NormalTok{(tab, }\AttributeTok{shade =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{las =} \DecValTok{1}\NormalTok{,}
           \AttributeTok{main =} \StringTok{"Ejemplo 5: Mosaicplot (independencia)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej5-plot-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assocplot}\NormalTok{(tab, }\AttributeTok{main =} \StringTok{"Ejemplo 5: Assocplot (residuales estandarizados)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej5-plot-2.pdf}}

\subsubsection{Ejemplo 6: Homogeneidad en los grupos sanguíneos}\label{ejemplo-6-homogeneidad-en-los-grupos-sanguuxedneos-1}
...
\begin{Highlighting}[]
\NormalTok{tab }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}
  \FunctionTok{c}\NormalTok{(}\DecValTok{32}\NormalTok{,}\DecValTok{11}\NormalTok{,}\DecValTok{7}\NormalTok{,}\DecValTok{2}\NormalTok{,}
    \DecValTok{47}\NormalTok{,}\DecValTok{13}\NormalTok{,}\DecValTok{17}\NormalTok{,}\DecValTok{9}\NormalTok{,}
    \DecValTok{23}\NormalTok{,}\DecValTok{7}\NormalTok{,}\DecValTok{9}\NormalTok{,}\DecValTok{6}\NormalTok{),}
  \AttributeTok{nrow =} \DecValTok{3}\NormalTok{, }\AttributeTok{byrow =} \ConstantTok{TRUE}
\NormalTok{)}
\FunctionTok{rownames}\NormalTok{(tab) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Caucásicos"}\NormalTok{,}\StringTok{"Pigmeos"}\NormalTok{,}\StringTok{"Esquimales"}\NormalTok{)}
\FunctionTok{colnames}\NormalTok{(tab) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"O"}\NormalTok{,}\StringTok{"A"}\NormalTok{,}\StringTok{"B"}\NormalTok{,}\StringTok{"AB"}\NormalTok{)}
\NormalTok{tab}
\end{Highlighting}
\end{Shaded}
...
## Sum        102 31 33 17 183
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# El test de homogeneidad se implementa igual que el test de independencia sobre la tabla de recuentos}
\NormalTok{test6 }\OtherTok{\textless{}{-}} \FunctionTok{chisq.test}\NormalTok{(tab, }\AttributeTok{correct =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{test6}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  tab
## X-squared = 4.6911, df = 6, p-value = 0.584
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test6}\SpecialCharTok{$}\NormalTok{expected}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{mosaicplot}\NormalTok{(tab, }\AttributeTok{shade =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{las =} \DecValTok{1}\NormalTok{,}
           \AttributeTok{main =} \StringTok{"Ejemplo 6: Mosaicplot (homogeneidad)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej6-plot-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assocplot}\NormalTok{(tab, }\AttributeTok{main =} \StringTok{"Ejemplo 6: Assocplot (residuales estandarizados)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{FundamentosInferenciaEstadistica_files/figure-latex/ej6-plot-2.pdf}}
...
\subsubsection{Ejemplo 10}\label{ejemplo-10}
...
\subparagraph{Ejemplo 14}\label{ejemplo-14}
...
\subparagraph{Ejemplo}\label{ejemplo-11}
...
\section{\texorpdfstring{Métodos de computación intensiva: El \emph{Bootstrap}}{Métodos de computación intensiva: El Bootstrap}}\label{muxe9todos-de-computaciuxf3n-intensiva-el-bootstrap}

\subsection{Introducción: Precisión de un estimador}\label{introducciuxf3n-precisiuxf3n-de-un-estimador}

Los métodos bootstrap que se expondrán a continuación son un conjunto de técnicas introducidas por Bradley Efron de la universidad de Stanford a finales de los años 70 ({[}7,9,12{]}) y que se han revelado como una poderosa herramienta de gran utilidad en diversos campos de la estadística.

\subsubsection{Ejemplos de introducción}\label{ejemplos-de-introducciuxf3n}
...
\paragraph{Problema 1: Tiempo de supervivencia}\label{problema-1-tiempo-de-supervivencia}
...
\textbar{} .14 \textbar{} .18 \textbar{} .25 \textbar{} .26 \textbar{} .37 \textbar{}
\textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{}
\textbar{} .43 \textbar{} .51 \textbar{} .61 \textbar{} .71 \textbar{} \textbar{}
...
\paragraph{Problema 2: Coeficiente de correlación}\label{problema-2-coeficiente-de-correlaciuxf3n}
...
\textbar{} Estudiante núm: \textbar{} 1 \textbar{} 2 \textbar{} 3 \textbar{} 4 \textbar{} 5 \textbar{} 6 \textbar{} 7 \textbar{} 8 \textbar{}
\textbar{} :--- \textbar{} :---: \textbar{} :---: \textbar{} :---: \textbar{} :---: \textbar{} :---: \textbar{} :---: \textbar{} :---: \textbar{} :---: \textbar{}
\textbar{} Selectividad \textbar{} 5.76 \textbar{} 6.35 \textbar{} 5.58 \textbar{} 5.78 \textbar{} 6.66 \textbar{} 5.8 \textbar{} 5.55 \textbar{} 6.61 \textbar{}
\textbar{} COU \textbar{} 6.78 \textbar{} 6.6 \textbar{} 5.62 \textbar{} 6.06 \textbar{} 6.88 \textbar{} 6.14 \textbar{} 6 \textbar{} 6.86 \textbar{}
\textbar{} Estudiante núm: \textbar{} 9 \textbar{} 10 \textbar{} 11 \textbar{} 12 \textbar{} 13 \textbar{} 14 \textbar{} 15 \textbar{} \textbar{}
\textbar{} Selectividad \textbar{} 6.51 \textbar{} 6.05 \textbar{} 6.53 \textbar{} 5.75 \textbar{} 5.45 \textbar{} 5.72 \textbar{} 5.94 \textbar{} \textbar{}
\textbar{} COU \textbar{} 6.72 \textbar{} 6.26 \textbar{} 6.24 \textbar{} 5.48 \textbar{} 5.52 \textbar{} 5.76 \textbar{} 5.92 \textbar{} \textbar{}
...
\subsubsection{Estimación del error estándar: soluciones clásica y bootstrap}\label{estimaciuxf3n-del-error-estuxe1ndar-soluciones-cluxe1sica-y-bootstrap}
...
\subsubsection{Estimadores de substitución (``plug-in'') y Funcionales estadísticos}\label{estimadores-de-substituciuxf3n-plug-in-y-funcionales-estaduxedsticos}

\paragraph{La distribución muestral (o empírica)}\label{la-distribuciuxf3n-muestral-o-empuxedrica}
...
\paragraph{Estimadores de substitución}\label{estimadores-de-substituciuxf3n}
...
\subsubsection{El bootstrap}\label{el-bootstrap}

\paragraph{Estimación de la distribución en el muestreo}\label{estimaciuxf3n-de-la-distribuciuxf3n-en-el-muestreo}
...
\subsubsection{Ejemplo 1 (continuación)}\label{ejemplo-1-continuaciuxf3n}
...
\subsubsection{Resumen: del mundo real al mundo bootstrap}\label{resumen-del-mundo-real-al-mundo-bootstrap}
...
\subsubsection{Otros aspectos}\label{otros-aspectos}

\paragraph{Bootstrap paramétrico y no paramétrico}\label{bootstrap-paramuxe9trico-y-no-paramuxe9trico}
...
\paragraph{Numero de remuestras necesario}\label{numero-de-remuestras-necesario}
...
\item
  \(\hat{\sigma}_{\hat{F}}(\hat{\theta})=\hat{\sigma}_{B}(\hat{\theta})\) a la aproximación de Monte Carlo del estimador bootstrap del error estándar de \(\hat{\theta}\),
...
\subsubsection{Practicas}\label{practicas}
...
\subsection{Estimación y corrección del sesgo de un estimador}\label{estimaciuxf3n-y-correcciuxf3n-del-sesgo-de-un-estimador}

\subsubsection{Introducción}\label{introducciuxf3n-10}
...
\subsubsection{Estimación bootstrap del sesgo}\label{estimaciuxf3n-bootstrap-del-sesgo}
...
\subsubsection{Ejemplo 1. Estimación del sesgo del estimador de la varianza}\label{ejemplo-1.-estimaciuxf3n-del-sesgo-del-estimador-de-la-varianza}
...
\subsubsection{Corrección del sesgo de un estimador}\label{correcciuxf3n-del-sesgo-de-un-estimador}
...
\subsubsection{El jackknife}\label{el-jackknife}
...
\paragraph{El estimador jackknife del sesgo}\label{el-estimador-jackknife-del-sesgo}
...
\subsubsection{Introducción}\label{introducciuxf3n-11}
...
\subsubsection{Intervalos de confianza estándar}\label{intervalos-de-confianza-estuxe1ndar}
...
\subsection{Intervalos de confianza basados en tablas bootstrap}\label{intervalos-de-confianza-basados-en-tablas-bootstrap}

\subsubsection{El caso de la t de Student}\label{el-caso-de-la-t-de-student}
...
\subsubsection{Intervalos de confianza basados en tablas bootstrap}\label{intervalos-de-confianza-basados-en-tablas-bootstrap-1}
...
\subsubsection{El bootstrap-t y las transformaciones}\label{el-bootstrap-t-y-las-transformaciones}
...
\paragraph{Transformación normalizante para el coeficiente de correlación}\label{transformaciuxf3n-normalizante-para-el-coeficiente-de-correlaciuxf3n}
...
\paragraph{\texorpdfstring{Aplicación del bootstrap- \(t\) sobre la escala transformada}{Aplicación del bootstrap- t sobre la escala transformada}}\label{aplicaciuxf3n-del-bootstrap--t-sobre-la-escala-transformada}
...
\subsection{Intervalos de confianza basados en percentiles bootstrap}\label{intervalos-de-confianza-basados-en-percentiles-bootstrap}

\subsubsection{Intervalos basados en percentiles de una distribución normal}\label{intervalos-basados-en-percentiles-de-una-distribuciuxf3n-normal}
...
\subsubsection{Los intervalos percentil}\label{los-intervalos-percentil}
...
\paragraph{Una situación en la que podemos saber qué intervalo es mejor}\label{una-situaciuxf3n-en-la-que-podemos-saber-quuxe9-intervalo-es-mejor}
...
\paragraph{La propiedad de respetar las transformaciones}\label{la-propiedad-de-respetar-las-transformaciones}
...
\textbar{} 1. Exactos (teoría normal) \textbar{} \([.496, .898]\) \textbar{} \(\mathrm{D} / \mathrm{I}=0.44\) \textbar{}
\textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{}
\textbar{} 2. Estándar \textbar{} \([.587, .965]\) \textbar{} \(\mathrm{D} / \mathrm{I}=1.00\) \textbar{}
\textbar{} 3. Estándar transformados \textbar{} \([.508, .907]\) \textbar{} \(\mathrm{D} / \mathrm{I}=.49\) \textbar{}
\textbar{} 4. Percentil ordinario \textbar{} \([.536, .911]\) \textbar{} \(\mathrm{D} / \mathrm{I}=.56\) \textbar{}
\textbar{} 5. Percentil corregido \((B C)\) \textbar{} \([.488, .900]\) \textbar{} \(\mathrm{D} / \mathrm{I}=.43\) \textbar{}
...
\textbar{} Cuatro métodos de construcción de IC aproximados para un parámetro \(\theta\) \textbar{} \textbar{} \textbar{} \textbar{} \textbar{}
\textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{} :--- \textbar{}
\textbar{} Método \textbar{} Abreviación \textbar{} Extremo de nivel \(\alpha\) \textbar{} Es correcto si \textbar{} \textbar{}
\textbar{} 1. Estándar \textbar{} \(\theta_{E}[\alpha]\) \textbar{} \(\hat{\theta}+\hat{\sigma}_{\hat{\theta}} z_{\alpha}\) \textbar{} \(\hat{\theta} \sim N\left(\theta, \sigma^{2}\right)\) \textbar{} \(\sigma\) constante \textbar{}
\textbar{} \textbar{} \textbar{} \textbar{} Existe una transformación monótona \(\hat{\phi}=g(\hat{\theta}), \phi=g(\theta)\), tal que: \textbar{} \textbar{}
\textbar{} 2. Percentil \textbar{} \(\theta_{P}[\alpha]\) \textbar{} \(\hat{G}^{-1}(\alpha)\) \textbar{} \(\hat{\phi} \sim N\left(\phi, \tau^{2}\right)\) \textbar{} \(\tau\) constante \textbar{}
\textbar{} 3. \(B C\) \textbar{} \(\theta_{B C}[\alpha]\) \textbar{} \(\hat{G}^{-1}\left(\Phi\left[2 z_{o}+z_{\alpha}\right]\right)\) \textbar{} \(\hat{\phi} \sim N\left(\phi-z_{o} \tau, \tau^{2}\right)\) \textbar{} \(z_{o}, \tau\) constante \textbar{}
\textbar{} 4. \(B C_{a}\) \textbar{} \(\theta_{B C_{a}}[\alpha]\) \textbar{} \(\hat{G}^{-1}\left(\Phi\left[z_{o}+\frac{z_{o}+z_{\alpha}}{1-a\left(z_{o}+z_{\alpha}\right)}\right]\right)\) \textbar{} \(\hat{\phi} \sim N\left(\phi-z_{o} \tau_{\phi}, \tau_{\phi}^{2}\right)\) \textbar{} \(\tau_{\phi}=1+a \phi\) \textbar{}
\textbar{} \textbar{} \textbar{} \textbar{} \textbar{} \textbar{}
...
\paragraph{Ejemplo: IC para la varianza de una población normal}\label{ejemplo-ic-para-la-varianza-de-una-poblaciuxf3n-normal}
...
Tabla 5
\textbar{} Intervalos de confianza exactos y aproximados al 90 \% para la varianza de una población normal \textbar{} \textbar{}
\textbar{} :--- \textbar{} :--- \textbar{}
\textbar{} 1. Exactos (teoría normal) \textbar{} \(\left[, 631 S^{2}, 1,88 S^{2}\right]\) \textbar{}
\textbar{} 2. Basados en la verosimilitud \textbar{} \(\left[, 466 S^{2}, 1,53 S^{2}\right]\) \textbar{}
\textbar{} 3. Percentil corregido ( \(B C\) ) \textbar{} {[},580S \({ }^{2}\), \(1,69 S^{2}\) {]} \textbar{}
\textbar{} 4. Percentil acelerado ( \(B C_{a}\) ) \textbar{} , \(630 S^{2}\), \(1,88 S^{2}\) {]} \textbar{}
...
\subsubsection{Prácticas}\label{pruxe1cticas}
...
\item
  Representar la distribución bootstrap. ¿Parece normal?
...
\item
  Remuestrear con \(\mathrm{B}=1000\)
\item
  Construir el IC estándar y el IC percentil.
...
\subsection{Contraste de hipótesis mediante bootstrap}\label{contraste-de-hipuxf3tesis-mediante-bootstrap}

\subsubsection{Introducción}\label{introducciuxf3n-12}
...
\paragraph{Revisión de conceptos básicos de contraste de hipótesis}\label{revisiuxf3n-de-conceptos-buxe1sicos-de-contraste-de-hipuxf3tesis}
...
\subsection{Elección del estadístico de test}\label{elecciuxf3n-del-estaduxedstico-de-test}
...
\subsection{Cálculo del p-valor}\label{cuxe1lculo-del-p-valor-1}
...
\subsubsection{Contrastes de hipótesis bootstrap}\label{contrastes-de-hipuxf3tesis-bootstrap}
...
\subsection{Ejemplo 1: Comparación de medias}\label{ejemplo-1-comparaciuxf3n-de-medias}
...
\tightlist
\item
  Generar \(b=1, \ldots, B\) pares de remuestras a partir de \(\widehat{F}_{H_{0}}\) definida en el párrafo anterior y calcular sobre cada una de ellas el estadístico de test
  \(T^{*}\)
\end{enumerate}
...
\subsection{Ejemplo 2: Prueba de independencia}\label{ejemplo-2-prueba-de-independencia}
...
\paragraph{\texorpdfstring{Resumiendo: Estimación de \(F\) bajo \(H_{0}\)}{Resumiendo: Estimación de F bajo H\_\{0\}}}\label{resumiendo-estimaciuxf3n-de-f-bajo-h_0}
...
\subsubsection{Relación entre contrastes de hipótesis, intervalos de confianza y el bootstrap}\label{relaciuxf3n-entre-contrastes-de-hipuxf3tesis-intervalos-de-confianza-y-el-bootstrap}
...
\paragraph{Conceptos básicos de intervalos de confianza}\label{conceptos-buxe1sicos-de-intervalos-de-confianza}
...
\paragraph{Relación entre los intérvalos de confianza y las pruebas de hipótesis}\label{relaciuxf3n-entre-los-intuxe9rvalos-de-confianza-y-las-pruebas-de-hipuxf3tesis}
...
\paragraph{Pruebas de hipótesis, intervalos de confianza y bootstrap}\label{pruebas-de-hipuxf3tesis-intervalos-de-confianza-y-bootstrap}
...
\subsection{Validez de los métodos bootstrap}\label{validez-de-los-muxe9todos-bootstrap}

\subsubsection{Introducción}\label{introducciuxf3n-13}
...
\subsubsection{Teorema central del límite bootstrap}\label{teorema-central-del-luxedmite-bootstrap}
...
\subsubsection{Validación del bootstrap mediante simulación}\label{validaciuxf3n-del-bootstrap-mediante-simulaciuxf3n}
...
\textbar{} Bootstrap \((\mathrm{B}=128)\) \textbar{} 0.197 \textbar{}
\textbar{} :--- \textbar{} :--- \textbar{}
\textbar{} Teoría normal \textbar{} 0.207 \textbar{}
\textbar{} Verdadero valor \textbar{} 0.208 \textbar{}
...
\bibitem[\citeproctext]{ref-canavos1988}
Canavos, George C. 1988. \emph{Probabilidad y Estadística: Aplicaciones y Métodos}. México: McGraw-Hill/Interamericana.

\bibitem[\citeproctext]{ref-casella1990}
Casella, George, and Roger L. Berger. 1990. \emph{Statistical Inference}. Duxbury Press.

\bibitem[\citeproctext]{ref-cuadras2000}
Cuadras, C. M. 2000. \emph{Problemas de Probabilidades y Estadística. Vol. 2: Inferencia Estadística}. Barcelona: EUB. Economía y Empresa.

\bibitem[\citeproctext]{ref-degroot1988}
De Groot, Morris. 1988. \emph{Probabilidad y Estadística}. Addison-Wesley.

\bibitem[\citeproctext]{ref-dudewicz1989}
Dudewicz, Edward J., and S. N. Mishra. 1989. \emph{Modern Mathematical Statistics}. Wiley Series in Probability and Statistics. New York: John Wiley \& Sons.

\bibitem[\citeproctext]{ref-fortiana1999}
Fortiana, Josep, and David Nualart. 1999. \emph{Estadística}. Barcelona: Publicacions de la Universitat de Barcelona.

\bibitem[\citeproctext]{ref-lehmann1986}
Lehmann, Erich L. 1986. \emph{Testing Statistical Hypotheses}. Wiley Series in Probability and Statistics. New York: John Wiley \& Sons.

\bibitem[\citeproctext]{ref-martinez1993}
Martínez, Antonio, Carlos Rodríguez, and Rafael Gutiérrez. 1993. \emph{Inferencia Estadística: Un Enfoque Clásico}. Madrid: Ediciones Pirámide.

\bibitem[\citeproctext]{ref-pena1987}
Peña, Daniel. 1987. \emph{Estadística: Modelos y Métodos i. Fundamentos}. Madrid: Alianza Editorial.

\bibitem[\citeproctext]{ref-rohatgi1976}
Rohatgi, V. K. 1976. \emph{An Introduction to Probability Theory and Mathematical Statistics}. Wiley Series in Probability. New York: John Wiley \& Sons.

\bibitem[\citeproctext]{ref-ruizmaya1995}
Ruiz-Maya, Luis, and Juan Martín Pliego. 1995. \emph{Estadística II: Inferencia}. Colección Plan Nuevo. Madrid: Editorial AC.

\bibitem[\citeproctext]{ref-sanz1999}
Sanz, Marta. 1999. \emph{Probabilitats}. Barcelona: Edicions de la Universitat de Barcelona.

\bibitem[\citeproctext]{ref-velez1993}
Vélez Ibarrola, Ricardo, and Alfonso García Pérez. 1993. \emph{Principios de Inferencia Estadística}. Madrid: Editorial UNED.
