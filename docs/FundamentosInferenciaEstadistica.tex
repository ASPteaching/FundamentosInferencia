...
\section{CONTRASTES DE LA JI-CUADRADO}\label{contrastes-de-la-ji-cuadrado}
...
\tightlist
\item
  Pruebas de bondad de ajuste (\emph{goodness of fit}) de unos datos a una distribución
...
\subsubsection{\texorpdfstring{Estadístico de test para las pruebas \(\chi^{2}\) de ajuste}{Estadístico de test para las pruebas \textbackslash chi\^{}\{2\} de ajuste}}\label{estaduxedstico-de-test-para-las-pruebas-chi2-de-ajuste}
...
\textbf{Teorema 1} \emph{Bajo las condiciones descritas más arriba, si las probabilidades \(p_{1}^{0}, \ldots, p_{k}^{0}\) son números conocidos, entonces el estadístico:}

\[
D^{2}=\sum_{i=1}^{k} \frac{\left(n_{i}-n p_{i}^{0}\right)^{2}}{n p_{i}^{0}} .
\]

\emph{tiene distribución asintótica \(\chi^{2}\) con \(k-1\) grados de libertad}
...
\textbf{Definición 1:} Test de bondad de ajuste
...
\subsubsection{Observaciones}\label{observaciones-2}
...
\textbf{Ejemplo 7.2.3} En el ejemplo (7.2.2), cuando hemos ajustado unos datos a unas proporciones del tipo ``1:2:1'' o ``9:3:3:1'' estábamos suponiendo dos cosas:
...
\textbf{Ejemplo 7.2.4} En un estudio sobre bilingüismo se aplicó una prueba a una muestra de 211 alumnos y se obtuvieron 211 puntuaciones que, una vez tabuladas, eran las siguientes:
...
\paragraph{Observaciones}\label{observaciones-3}
...
\subsubsection{Observaciones}\label{observaciones-7}
...
\subparagraph{Ejemplo}\label{ejemplo-15}
