...
muchas veces una misma experiencia en condiciones independientes e
idénticamente distribuidas, la frecuencia relativa de un suceso fijo se
aproxima a su probabilidad. Esta es una propiedad asintótica, no una
...
A esta función la denominaremos función de probabilidad. Formalmente,
en un espacio probabilizable $(\Omega,\mathcal A)$ es una aplicación

$$
P:\mathcal A\longrightarrow [0,1].
$$

Aquí $\mathcal A$ es la colección de sucesos a los que se puede asignar
probabilidad. Formalmente es una *sigma-álgebra*: contiene $\Omega$ y es
estable frente a complementos y uniones numerables. En espacios finitos
suele tomarse $\mathcal A=\mathcal P(\Omega)$, el conjunto de todos los
...
Y, ¿cuáles son estos axiomas? Pues los siguientes, para sucesos de
$\mathcal A$:
...
y la regla que les asigna probabilidades ($P$). En el caso finito habitual,
todos los subconjuntos de $\Omega$ pueden considerarse sucesos.
::::
...
elementales**, que son los elementos de $\Omega$, de los **sucesos medibles**,
que son los elementos de $\mathcal A$. En un dado ordinario podemos observar
cada número y tomar todos sus subconjuntos como sucesos. Si el modo de
observación es más limitado, no todos los subconjuntos tienen por qué estar en
$\mathcal A$.
...
Para formalizar estos conceptos, el par $(\Omega,\mathcal A)$ se denomina
**espacio medible** (o espacio probabilizable). $\mathcal A$ recoge los
sucesos medibles y debe ser una sigma-álgebra. En el dado pintado con tres
colores, por ejemplo, los colores son átomos observables y la sigma-álgebra
generada por ellos contiene $\varnothing$, los tres colores, sus uniones y
$\Omega$; no contiene necesariamente el suceso «sale par».
...
cota útil puede truncarse en cero. La terna $(\Omega,\mathcal A,P)$ definida
arriba es el **espacio de probabilidad**: $P$ solo se asigna a sucesos de
$\mathcal A$.
...
En términos generales, si $A,B\in\mathcal A$ y $P(B)>0$, definimos la
probabilidad condicionada como:
...
Para un $B$ fijo con $P(B)>0$, la aplicación $P_B(A)=P(A\mid B)$ es una
medida de probabilidad sobre $\mathcal A$ y verifica los tres axiomas:
